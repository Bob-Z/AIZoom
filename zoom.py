...
def generate_missing_part(image, inpaint_pipeline, prompt):
    ...
    result = inpaint_pipeline(prompt=prompt, image=image_pil, mask_image=mask_pil, strength=1, guidance_scale=7.5, num_inference_steps=50).images[0]
...
def overlay_images(source_image, inpainted_image, mask):
    ...
    for c in range(0, 3):
        combined[:, :, c] = (alpha_source * source_image_rgba[:, :, c] + alpha_inpainted * inpainted_image_rgba[:, :, c])
...
def main(prompt, output_dir, num_iterations=5):
    # Check if GPU is available and set the device
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # Load the Stable Diffusion model for generating the initial image
    generation_pipeline = StableDiffusionPipeline.from_pretrained("CompVis/stable-diffusion-v1-4", safety_checker=None, requires_safety_checker=False).to(device)
    
    # Generate the initial image using the prompt
    with torch.autocast("cuda"):
        initial_image = generation_pipeline(prompt, guidance_scale=7.5, num_inference_steps=50).images[0]
    ...
    for i in range(num_iterations):
        ...
        final_image, mask = generate_missing_part(centered_image, inpaint_pipeline, prompt)
        ...
        output_image.save(f"{output_dir}/output_image_{i}.png", format='PNG')  # Explicitly specify the format
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("Usage: python script.py <prompt> <output_dir> <num_iterations>")
        sys.exit(1)

    prompt = sys.argv[1]
    output_dir = sys.argv[2]
    num_iterations = int(sys.argv[3])
    main(prompt, output_dir, num_iterations)
